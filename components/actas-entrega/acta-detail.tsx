...
import { AlertTriangle, ArrowLeft, Camera, CheckCircle2, Download, FileSignature, PackageCheck, Trash2, XCircle } from "lucide-react";
...
export function ActaDetail({ admin = false, readOnly = false }: { admin?: boolean; readOnly?: boolean }) {
  ...
  return (
    <div className="relative z-10 mx-auto max-w-6xl space-y-6 p-4 md:p-8">
      ...
      {acta.actas_entrega_firmas.length > 0 && <Card className="border-0 shadow-md"><CardHeader><CardTitle className="text-base">Firmas registradas</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">{acta.actas_entrega_firmas.map((firma: any) => <div key={firma.id} className="rounded-lg border p-4"><p className="mb-2 text-sm font-semibold capitalize">{firma.rol_firmante}</p><img src={firma.url} alt={`Firma del ${firma.rol_firmante}`} className="h-24 max-w-full object-contain" /><p className="mt-2 text-xs text-muted-foreground">Firmada el {new Date(firma.firmada_at).toLocaleString("es-CO")}</p></div>)}</CardContent></Card>}
    </div>
  );
}
