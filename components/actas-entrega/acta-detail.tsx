...
export function ActaDetail({ admin = false, readOnly = false }: { admin?: boolean; readOnly?: boolean }) {
  ...
  const signerDocument = isReceiver ? acta.receptor_documento : acta.entregante_documento;
  ...
  return (
    <div className="relative z-10 mx-auto max-w-6xl space-y-6 p-4 md:p-8">
      ...
      {!readOnly && isSender && acta.estado === "borrador" && <Card className="border-amber-200 shadow-lg"><CardHeader><CardTitle className="flex items-center gap-2"><FileSignature className="h-5 w-5 text-[#6b2b16]" />Firma y envío</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-muted-foreground">Al firmar, los datos y elementos quedarán bloqueados y el receptor será notificado.</p>{!acta.entregante_documento && <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">Debes tener una cédula registrada para firmar esta acta.</div>}<SignaturePad onChange={setSignature} signerName={acta.entregante_nombre} signerDocument={acta.entregante_documento} /><label className="flex items-start gap-3 rounded-lg bg-stone-50 p-4 text-sm"><Checkbox checked={consent} onCheckedChange={(value) => setConsent(value === true)} /><span>Declaro que los elementos relacionados corresponden a la entrega realizada y acepto firmar electrónicamente esta acta.</span></label><Button className="w-full bg-[#441404] hover:bg-[#5c1d0a]" size="lg" disabled={submitting || !acta.entregante_documento} onClick={send}>{submitting ? "Enviando..." : "Firmar y enviar al receptor"}</Button></CardContent></Card>}

      {canReceive && <Card className="border-amber-200 shadow-lg"><CardHeader><CardTitle className="flex items-center gap-2"><FileSignature className="h-5 w-5 text-[#6b2b16]" />Decisión y firma de recibido</CardTitle></CardHeader><CardContent className="space-y-4"><div className="rounded-lg bg-amber-50 p-4 text-sm text-amber-900"><AlertTriangle className="mr-2 inline h-4 w-4" />Revisa todos los elementos. Los estados distintos de “Bueno” requieren una nota; cada elemento recibido requiere una fotografía.</div>{!acta.receptor_documento && <div className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">Debes tener una cédula registrada para firmar esta acta.</div>}<SignaturePad onChange={setSignature} signerName={acta.receptor_nombre} signerDocument={acta.receptor_documento} /><label className="flex items-start gap-3 rounded-lg bg-stone-50 p-4 text-sm"><Checkbox checked={consent} onCheckedChange={(value) => setConsent(value === true)} /><span>{acta.manifiesto ? "Declaro que he leído y acepto el manifiesto de responsabilidad de esta acta, y que la información de recepción registrada es veraz." : "Declaro que revisé los elementos, registré su estado real y acepto firmar electrónicamente esta acta de recibido."}</span></label><div className="flex flex-col gap-3 sm:flex-row"><Button variant="destructive" className="sm:w-1/3" onClick={() => setShowReject((value) => !value)}>Rechazar entrega</Button><Button className="bg-emerald-700 hover:bg-emerald-800 sm:flex-1" disabled={submitting || !signerDocument} onClick={accept}><CheckCircle2 className="mr-2 h-4 w-4" />{submitting ? "Procesando..." : "Firmar y aceptar entrega"}</Button></div>{showReject && <div className="space-y-3 rounded-xl border border-red-200 bg-red-50 p-4"><Label>Motivo del rechazo *</Label><Textarea value={rejectReason} onChange={(event) => setRejectReason(event.target.value)} placeholder="Explica claramente por qué no puedes aceptar la entrega" /><Button variant="destructive" disabled={submitting || rejectReason.trim().length < 10} onClick={reject}>Confirmar rechazo</Button></div>}</CardContent></Card>}
