...
export function ComentariosVacaciones({ solicitudId, isAdmin }: { solicitudId?: string; isAdmin?: boolean }) {
  ...
  useEffect((): (() => void) => {
    ...
    const channel = supabase
      .channel("realtime_comentarios_vacaciones")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "comentarios_vacaciones", filter: `solicitud_id=eq.${solicitudId}` },
        () => {
          fetchComentarios()
          fetchUnreadCount()
        }
  ...
  const handleComentar = async () => {
    ...
    setLoading(true)
    await supabase.from("comentarios_vacaciones").insert({
    ...
    <div ref={rootRef} className="bg-white rounded-lg shadow">
      <div className="border-b px-4 py-3 flex items-center justify-between">
        <h2 className="text-lg font-semibold flex items-center gap-2">
          ...
          <span>Comentarios de vacaciones</span>
        </h2>
      </div>
